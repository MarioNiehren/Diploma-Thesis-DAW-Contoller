 ...
 * - replace <strong> #include "stm32f1xx_hal.h" </strong> in EPD_1-54inch.h with the hal header of the used STM32 MCU if it does not fit.
 * - Only STM32F103C8T6 is tested. Others should work, but there is no guarantee.
 *
 * ## Links:
...
typedef struct
{
  ...
  Buffer_StructTd BufferManagement;
 ...
 *
 * 						## HAL Settings for SPI (as tested):
 ...
 * 								- CPHA: 2 Edge
 * 						-	NVIC Settings:
 * 							- Enable Interrupt for DMA and SPI
 * 						-	DMA Settings
 * 							- DMA Request:	SPI1_TX
 ...
 * @warning		If you only use partial refresh, it may lead to ghost images and
 ...
 * 						implemented yet. If you have problems with ghost images, try to
 * 						refresh the screen in full mode for a few times with a complete
 * 						black or white frame. Depending on how intense the ghost image
