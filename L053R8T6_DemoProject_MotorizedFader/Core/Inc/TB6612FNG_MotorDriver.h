 ...
 *
 * # How to use:
 * 1. Initialize Pin In1, Pin In2, Pin STBY
 * 2. Initialize PWM (check MotorDriver_init_PWM() for information about the TIM
 *    settings.)
 * 3. Start PWM -> The PWM will be generated continuous now.
 * 4. Use move, stop or standby functions to control the motor.
 *
 *
 * # General information
 * -  @anchor PWM_Quick_Explanation PWM Quick Explanation
 *  - The PWM is generated by an internal counter that counts up on each clock
 *    cycle of the used TIMx until a specific value is reached. The counter
 *    restarts when the value is reached. This value is the auto reload register
 *    (TIMx_ARR). The ARR value determines the period length. The Pulse width is
 *    determined by TIMx_CCRx value. CCR means  capture/compare register. The
 *    PWM Pulse will be high until the up counter reached the CCR value.
 *    Until ARR is reached, the PWM pulse will be low.
 *    For this module: The most relevant value is CCR, as it is used to control
 *    the PWM duty cycle in the running program.
 *
 * # Links:
...
typedef struct
{
  ...
  uint16_t      CCR;          /**<  current CCR value for the PWM  */
 ...
 *
 * # How to setup PWM in CubeMX (as tested):
 *
 * - HCLK: 32MHz
 *
 * - Setup Channel x: PWM Generation CHx  (choose a free channel)
 *
 * - DMA/NVIC: Disable
 *
 * - Parameter Settings
 *  - Counter Settings
 *    - Prescaler:    1-1 (1 is the actual value, -1 is required for the way
 *                    the microcontroller counts. Check TIMx_PSC register in
 *                    reference manual for details.)
 *    - Counter Mode:                           Up
 *    - Counter Period (Auto Reload Register):  500 - 1
 ...
 *
 * -  TIM-CLK = ClkIn / (Prescaler  + 1), where ClkIn is usually the clock of
 *    the timers AHPB bus. Often it is equal to HCLK depending on the other used
 *    peripherals. + 1 is already taken in account in the setup in cubeMX
 *    (here: Prescaler: 1-1).
 * -  PWM-Freuqency = (TIM-CLK / AutoReloadRegister)
 *
 * @param     Motor     pointer to the users motor structure of the motor used
 *                      with this IC.
